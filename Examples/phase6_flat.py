...
input_layer = InputPopulation(shape=(4,), dt=dt, additive_spike_trace=False)
first_layer = LIFPopulation(shape=(2,), v_th=-50.0, v_rest=-70.0, v_reset=-75.0, tau_m=50, r_m=10, t_ref=0,
                       init_t_ref=0, dt=dt, additive_spike_trace=False)
...
connection = DenseConnection(input_layer, first_layer, w=torch.rand(input_layer.s.numel(), first_layer.s.numel()) * 1, learning_rule=FlatSTDP, lr=[0.1, 0.1], weight_decay=0, window=100)
...
fig.show()
fig.savefig('flatstdp_single_10_1_1_long_.png', bbox_inches='tight')
# plt.rcParams["figure.figsize"] = 14, 20
# fig = plt.figure()
