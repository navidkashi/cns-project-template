...
input_layer = InputPopulation(shape=(4,), dt=dt, additive_spike_trace=False, tau_s=10)
...
connection = DenseConnection(input_layer, first_layer, w=torch.rand(input_layer.s.numel(), first_layer.s.numel()) * 0.5,
                             learning_rule=FlatRSTDP, lr=[0.05, 0.05], tau_c=10, weight_decay=0, wmax=4)
connection_monitor = Monitor(connection, state_variables=["w"])

net = Network(learning=True, dt=dt, reward=RawReward)
...
fig = plt.figure()
axs = fig.subplots(5, gridspec_kw={'hspace': 0.5})
plot_spikes(axs[0], 'Input Spikes', s_input, dt)
plot_spikes(axs[1], 'Output Spikes', s_first, dt)
plot_value(axs[2], 'Dopamine', dopamine, dt)
plot_weights(axs[3], 'Post neuron 0 weights', w[:, :, 0].reshape(time, -1), dt)
plot_weights(axs[4], 'Post neuron 1 weights', w[:, :, 1].reshape(time, -1), dt)
...
fig.show()
fig.savefig('flat_05_10_20.png', bbox_inches='tight')
